...
export const generateOutfitCombinations = (keepItems, occasion = 'casual') => {
  if (!Array.isArray(keepItems) || keepItems.length === 0) {
    return [];
  }
  ...
  for (let i = 0; i < maxOutfits; i++) {
    ...
    // Ensure different items if possible
    if (keepItems.length > 1) {
...
export const calculateStatistics = (wardrobeItems, recommendations) => {
  if (!wardrobeItems.length) return { totalItems: 0, keep: 0, donate: 0, sell: 0 };
